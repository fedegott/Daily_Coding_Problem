...
""" I increase the difficulty of the above problem where I need to retrieve all words beginning with a given query such as 'tor' from https://www.merriam-webster.com/browse/thesaurus/, webscraping the website"""
...
def find_query(query,url):
    ...
    for i in range(pages+1):
        ...
        if i >0:
            entries_total.append(str(entries.text).split()) # this generates a list of list with all the words. entries_total[1][3] the index 1 is the page number and index 3 is the 3rd word in that page

# word = [[ i for i in k] for k in entries_total] # this is list comprehension to loop over list of lists

    results=[]
    for j in entries_total: # use 2 nested loops instead of list comprehension
        for k in j:
